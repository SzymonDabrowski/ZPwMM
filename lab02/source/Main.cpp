...
int iTerrainWidth, iTerrainHeight;
...
void PrepareHighMap()
{
  pHighMap = ReadBmpFromFile("..//..//source//terrain.bmp", iTerrainWidth, iTerrainHeight);
}

void DrawHighMap(float scale)
{
  glPushMatrix();
  ...
  for (int x = 0; x < iTerrainWidth - 1; x++)
  {
    for (int y = 0; y < iTerrainHeight - 1; y++)
    {
      //wektor normalny
      float xx1 = x + 1 - x;
      float yy1 = y - y;
      float zz1 = pHighMap[(y * iTerrainWidth + x + 1) * 3 + 0] / 4.0 - pHighMap[(y * iTerrainWidth + x + 1) * 3 + 0] / 4.0;

      float xx2 = x - x;
      float yy2 = y + 1 - y;
      float zz2 = pHighMap[((y + 1) * iTerrainWidth + x + 1) * 3 + 0] / 4.0 - pHighMap[(y * iTerrainWidth + x + 1) * 3 + 0] / 4.0;

      float xx3 = yy1 * zz2 - zz1 * yy2;
      float yy3 = xx1 * zz2 - zz1 * xx2;
      float zz3 = xx1 * yy2 - yy1 * xx2;
      float d = sqrt(xx3*xx3 + yy3*yy3 + zz3*zz3);

      glNormal3f(xx3/d,yy3/d,zz3/d);

      glVertex3f(x, y, pHighMap[(y * iTerrainWidth + x) * 3 + 0] / 4.0);
      glVertex3f(x + 1, y, pHighMap[(y * iTerrainWidth + x + 1) * 3 + 0] / 4.0);
      glVertex3f(x + 1, y + 1, pHighMap[((y + 1) * iTerrainWidth + x + 1) * 3 + 0] / 4.0);
      glVertex3f(x, y + 1, pHighMap[((y + 1)  * iTerrainWidth + x) * 3 + 0] / 4.0);
    }
  }
...
void DrawScene()
{
  ...
  GLfloat fLightPosition[] = { 10.0,1.0,1.0,0.0 };
  ...
  float scale = 0.02;
  glRotatef(90, -1, 0, 0);
	glTranslatef((-512/2)*scale, (-512/2)*scale, -1.5);
  DrawHighMap(scale);
  glPopMatrix();
}
